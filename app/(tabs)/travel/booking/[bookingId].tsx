    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: Math.max(insets.top, 16) }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summaryGrid}>
          <SummaryTile label="Travel Date" value={booking.travelDate} icon="event" />
          <SummaryTile label="Amount" value={booking.amount} icon="payments" />
          <SummaryTile label="Reference" value={booking.bookingId} icon="confirmation-number" />
          <SummaryTile label="Review" value="Admin verification" icon="verified-user" />
        </View>

        <View style={styles.actions}>
          <Pressable style={styles.primaryButton} onPress={() => router.push(`/(tabs)/travel/booking-status/${booking.bookingId}` as never)}>
            <Text style={styles.primaryButtonText}>View Status</Text>
            <MaterialIcons name="arrow-forward" size={18} color="#fff" />
          </Pressable>
          <Pressable style={styles.secondaryButton} onPress={() => router.push(`/(tabs)/travel/upload-documents/${booking.bookingId}` as never)}>
